...
#
# Single image that serves every network transport of the `ioc` VGI worker:
...
#
# Like vgi-units this worker is STATELESS: IOC extraction is pure text
# processing (regex over UTF-8) with the pattern set compiled into the binary,
# so there is no /data volume, no state registry, and no
...
# runtime ships. The workspace pins `vgi` from crates.io (no git patch), so the
# preinstalled toolchain is all that's needed.
...
RUN --mount=type=cache,target=/usr/local/cargo/registry \
    ...
    cargo build --release --locked --bin ioc-worker \
    && cp target/release/ioc-worker /ioc-worker
...
ARG SOURCE_URL=https://github.com/Query-farm/vgi-ioc
...
LABEL org.opencontainers.image.title="vgi-ioc" \
      org.opencontainers.image.description="Extract and defang/refang cyber-threat indicators of compromise (IOCs) from free text as a VGI worker for DuckDB/SQL (stdio + HTTP + TCP)" \
...
COPY --from=build --chmod=0755 /ioc-worker /usr/local/bin/ioc-worker
