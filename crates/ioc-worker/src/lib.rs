//! The `ioc` VGI worker (library) — defensive CTI tooling.
//!
...
//! the `ioc-wasm` crate (the browser build, which serves the same `Worker` over
...
/// The catalog name DuckDB sees in `ATTACH 'ioc' (TYPE vgi, …)`. Defaults to
/// `ioc`, but honors an explicit override so a test harness can rename it.
