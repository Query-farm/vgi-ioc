# CI: the vgi-ioc worker integration suite
...
(`test/sql/*.test`) against the vgi-ioc VGI worker through the **real DuckDB
...
1. **Build the worker** — `cargo build --release --bin ioc-worker`. The
   compiled `target/release/ioc-worker` is a self-contained stdio worker the
   extension spawns (the `.test` files `ATTACH` it via `${VGI_IOC_WORKER}`).
...
4. **Run** — [`run-integration.sh`](run-integration.sh) stages the preprocessed
   tree, points `VGI_IOC_WORKER` at the release binary, warms the
...
```bash
cargo build --release --bin ioc-worker
HAYBARN_UNITTEST=/path/to/haybarn-unittest \
VGI_IOC_WORKER="$PWD/target/release/ioc-worker" \
