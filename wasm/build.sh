#!/usr/bin/env bash
# Build the browser (`worker:`) build of the ioc VGI worker: ioc-wasm compiled
...
#     for the nightly toolchain, and the wasm32-unknown-emscripten target.
set -euo pipefail
cd "$(dirname "$0")/.."
...
LIB="target/$TARGET/release/libioc_wasm.a"
...
#
# OPT_LEVEL=3 overrides the workspace release profile for this target only.
#
...
# which the Arrow encoders + RPC layer overflow; an overflow surfaces as a bare
