...
#
# Dispatch the single vgi-ioc image into one of its transports:
...
#
# The worker is stateless (pattern set compiled into the binary), so there is no
...
case "${1:-http}" in
  http)
    ...
    exec ioc-worker --http "$@"
    ;;
  tcp)
    shift 2>/dev/null || true
    exec ioc-worker --tcp "0.0.0.0:${PORT_TCP:-8001}" "$@"
    ;;
  stdio)
    shift 2>/dev/null || true
    exec ioc-worker "$@"
